...
from dinov3.data.masking import InformationAwareMaskingGenerator
...
def collate_data_and_cast(
    ...
    # InformationAwareMaskingGenerator 인 경우 이미지 텐서를 전달
    _is_info_aware = isinstance(mask_generator, InformationAwareMaskingGenerator)

    for i in range(0, n_samples_masked):
        prob_max = probs[i + 1]
        num_mask = int(N * prob_max)
        if _is_info_aware:
            # collated_global_crops[i] = i번째 (crop × sample) 이미지
            img_tensor = collated_global_crops[i] if i < len(collated_global_crops) else None
            mask = torch.BoolTensor(mask_generator(num_mask, image_tensor=img_tensor))
        else:
            mask = torch.BoolTensor(mask_generator(num_mask))
    ...
    for _ in range(n_samples_masked, B):
        if _is_info_aware:
            masks_list.append(torch.BoolTensor(mask_generator(0, image_tensor=None)))
        else:
