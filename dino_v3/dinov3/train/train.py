...
from dinov3.data import (
    InformationAwareMaskingGenerator,
    MaskingGenerator,
    SamplerType,
...
def build_data_loader_from_cfg(
    ...
    random_mask_generator = MaskingGenerator(
        input_size=grid_size,
        max_num_patches=0.5 * img_size // patch_size * img_size // patch_size,
    )

    # information_aware_masking: true 이면 패치 정보량 기반 마스킹 활성화
    use_info_masking = getattr(cfg.ibot, "information_aware_masking", False)
    if use_info_masking:
        masking_temperature = getattr(cfg.ibot, "masking_temperature", 0.5)
        mask_generator = InformationAwareMaskingGenerator(
            input_size=grid_size,
            temperature=masking_temperature,
            fallback_generator=random_mask_generator,
        )
        logger.info(
            f"[iBOT] Information-aware masking 활성화 (temperature={masking_temperature})"
        )
    else:
        mask_generator = random_mask_generator
...
def do_train(cfg, model, resume=False):
    ...
    # TensorBoard
    tb_writer = None
    if _TENSORBOARD_AVAILABLE and distributed.is_main_process():
        tb_log_dir = os.path.join(cfg.train.output_dir, "tensorboard")
        tb_writer = SummaryWriter(log_dir=tb_log_dir)
        logger.info(f"TensorBoard logging: {tb_log_dir}")
    # 자동 GC 비활성화 (예측불가 pause 방지) + 수동 수거 주기 단축
    # WebDataset PIL 순환참조 누수 방지를 위해 150 → 20 iteration으로 단축
    gc.disable()
    gc.collect()
    ...
    for data in metric_logger.log_every(
        ...
        # Garbage collection + malloc_trim — worker에서 gc.enable()로 순환참조는 자체 수거.
        # 메인 프로세스는 collate/모델 연산의 잔여 객체만 처리.
        # malloc_trim: glibc가 freed 메모리를 OS에 반환하지 않아 RSS가 증가하는 문제 해결.
        # numpy/PIL의 반복적 alloc/free로 인한 힙 단편화를 주기적으로 정리.
        if (iteration + 1) % 20 == 0:
            gc.collect()
            try:
                ctypes.CDLL("libc.so.6").malloc_trim(0)
            except OSError:
                pass  # non-glibc 환경 (macOS 등) 에서는 무시
        ...
        if tb_writer is not None:
            tb_writer.add_scalar("train/total_loss", total_loss.item(), it)
            tb_writer.add_scalar("train/lr", lr, it)
            tb_writer.add_scalar("train/wd", wd, it)
            tb_writer.add_scalar("train/momentum", mom, it)
            tb_writer.add_scalar("train/teacher_temp", teacher_temp, it)
            for k, v in metrics_dict.items():
                tb_writer.add_scalar(f"train/{k}", v.item() if hasattr(v, "item") else v, it)
