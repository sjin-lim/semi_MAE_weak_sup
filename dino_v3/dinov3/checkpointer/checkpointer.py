...
def load_checkpoint(
    ...
    **others: Stateful,
    ...
    # ── 디버그: 체크포인트 로드 전 키 목록 기록 ──
    model_keys_before = set(to_load["model"].keys())
    logger.info(f"[ckpt-debug] model keys count before load: {len(model_keys_before)}")
    ...
    # ── 디버그: 체크포인트 로드 후 키 비교 ──
    model_keys_after = set(to_load["model"].keys())
    missing_in_ckpt = model_keys_before - model_keys_after
    extra_in_ckpt = model_keys_after - model_keys_before
    if missing_in_ckpt:
        logger.warning(f"[ckpt-debug] 체크포인트에 없어서 복원 안 된 키 ({len(missing_in_ckpt)}개): "
                       f"{sorted(missing_in_ckpt)[:20]}")
    if extra_in_ckpt:
        logger.warning(f"[ckpt-debug] 모델에 없는 체크포인트 키 ({len(extra_in_ckpt)}개): "
                       f"{sorted(extra_in_ckpt)[:20]}")
    if not missing_in_ckpt and not extra_in_ckpt:
        logger.info(f"[ckpt-debug] 모든 키 정상 매칭 ({len(model_keys_after)}개)")

    # teacher/EMA 가중치 복원 검증 — NaN 잔존 여부 확인
    teacher_keys = [k for k in model_keys_after if "teacher" in k or "model_ema" in k]
    logger.info(f"[ckpt-debug] teacher/EMA 관련 키: {len(teacher_keys)}개")

    dcpsd.set_model_state_dict(model, to_load["model"])

    # set_model_state_dict 후 NaN 잔존 체크 (init_weights가 못 덮은 경우 탐지)
    nan_params = []
    ...
    if nan_params:
        logger.error(f"[ckpt-debug] 체크포인트 로드 후 NaN 잔존 파라미터 ({len(nan_params)}개): "
                     f"{nan_params[:20]}")
    else:
        logger.info("[ckpt-debug] NaN 잔존 파라미터 없음 — 모든 가중치 정상 복원")

    if optimizer is not None:
        dcpsd.set_optimizer_state_dict(model, optimizer, to_load["optimizer"])
        # optimizer state 복원 확인
        opt_state_count = sum(1 for pg in optimizer.param_groups for _ in pg["params"])
        opt_state_loaded = len(optimizer.state)
        logger.info(f"[ckpt-debug] optimizer: {opt_state_loaded}/{opt_state_count} 파라미터 state 복원")
